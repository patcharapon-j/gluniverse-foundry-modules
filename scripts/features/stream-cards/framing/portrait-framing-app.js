...
import { isDirectorUser } from "../../stream/settings.js";
import { CARD_FLAGS, getDefaultRollArt, setDefaultRollArt } from "../settings.js";
...
class PortraitFramingApp extends ApplicationV2 {
  ...
  async saveOverride(focus) {
    ...
    if (entry.isDefaultArt) {
      ...
      await setDefaultRollArt({ ...getDefaultRollArt(), focus: saved });
    } else {
      ...
      const others = (actor.getFlag(MODULE_ID, CARD_FLAGS.portraitFocus) ?? []).filter(o => o?.src !== entry.src);
      const next = saved ? [...others, { src: entry.src, ...saved }] : others;
      if (next.length) await actor.setFlag(MODULE_ID, CARD_FLAGS.portraitFocus, next);
      else await actor.unsetFlag(MODULE_ID, CARD_FLAGS.portraitFocus);
    }
...
function collectEntries(pinned = new Set()) {
  ...
  const add = (actor, token, isNpc, player) => {
    ...
    const override = (base.getFlag(MODULE_ID, CARD_FLAGS.portraitFocus) ?? []).find(o => o?.src === src && isFocus(o)) ?? null;
