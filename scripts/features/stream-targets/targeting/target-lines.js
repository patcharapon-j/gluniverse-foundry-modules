import { getActiveCombatant, getActiveSceneCombat, getCombatantToken } from "../../stream/combat-utils.js";
import { HOOKS, MODULE_ID, TARGET_LINE_VISIBILITY } from "../../stream/constants.js";
...
export class TargetLineController {
  ...
  registerHooks() {
    ...
    for (const hook of [
      ...
      "sightRefresh",
      "userConnected",
      HOOKS.streamModeChanged,
      TARGETS_HOOKS.settingsChanged
    ]) Hooks.on(hook, refresh);
    Hooks.on("updateToken", (_doc, changes) => {
      if ("hidden" in changes || "disposition" in changes) refresh();
    });
    // `stream`'s own settings still matter: which login is the stream decides
    // the gmAndStream / streamOnly audiences.
    Hooks.on(HOOKS.settingsChanged, key => {
      if (key === "streamUserId") refresh();
    });
    if (canvas?.ready) {
      this.#createLayer();
      this.refresh();
