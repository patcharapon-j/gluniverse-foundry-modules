...
export function registerSettings() {
    ...
    // Master strength: blends the whole graded result back toward the original
    // art. Every layer is set explicitly by the GM, so full strength is the
    // natural default — the dial exists to back the whole look off at once.
    ...
    // The grade a scene starts from before the GM has graded it. Edited from
    // the Stage Director's Grade tab, never from Foundry's settings sheet — it is
    // a whole layer stack, not a value a text box can hold. Scenes store their
    // own stack in the `stage.grade` flag; see postfx/grade-store.mjs.
    game.settings.register(MODULE_ID, k('gradeDefaults'), {
        scope: 'world',
        config: false,
        type: Object,
        default: {},
