...
import { CLASSES, FEATURE_ID, HOOKS, MODULE_ID } from "./constants.js";
import { emitClientStatus } from "./socket.js";
import { requestAutoStart } from "./director-auth.mjs";
...
export class StreamMode {
  ...
  activate({ notify = true } = {}) {
    ...
    Hooks.callAll(HOOKS.streamModeChanged, true);
  ...
  deactivate({ notify = true } = {}) {
    ...
    Hooks.callAll(HOOKS.streamModeChanged, false);
  ...
  toggleRestore() {
    ...
    Hooks.callAll(HOOKS.restoreToggled, this.restoreVisible);
